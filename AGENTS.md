...
Six P1 ce jour-là, dans les deux sens. **Aucun ne portait sur du code faux** : tous portaient sur un chemin qu'aucun test ne traversait, ou sur un test vert pour une raison étrangère à ce qu'il prétendait vérifier. C'est la classe d'erreur qu'on ne voit pas seul, et c'est pourquoi la contre-revue existe.

- **Une assertion « aucun élément mauvais » doit prouver qu'elle avait des éléments à vérifier.** Le garde de CB-43 balaye les champs de charge de la page et exige qu'aucun ne déborde. Quand l'accueil de CB-63 a mis un écran devant l'éditeur, il a trouvé **zéro champ** — et il est resté vert, en ne protégeant plus rien. Il protégeait la troncature qu'Ugo avait trouvée lui-même en salle. Toute assertion sur un ensemble filtré (`toEqual([])`, `every`, `not.toContain`) s'accompagne d'une assertion sur la **taille de l'ensemble de départ**.
...
- **Lire la sortie de `check` jusqu'au bout avant de pousser.** Un `format:check` rouge arrête la chaîne avant les tests : la sortie ne contient alors **aucune ligne de test**, ce qui ressemble de loin à un succès silencieux. Vérifier la présence du compte de tests, pas l'absence d'erreurs.
